...
void applyPauliYFloatInternalOp(__m512 &v) {
...
void applyPauliYFloatExternal(std::complex<float> *arr,
                                     const size_t num_qubits,
    ...
    for (size_t k = 0; k < exp2(num_qubits - 1);
        ...
        _mm512_store_ps(arr + i0, Util::productImagS(v1, _mm512_set1_ps(-1.0F)));
        _mm512_store_ps(arr + i1, Util::productImagS(v0));
    }
}

template <size_t rev_wire>
static void applyPauliYDoubleInternalOp(__m512d &v) {
    if constexpr (rev_wire == 0) {
        const auto factor = _mm512_setr_pd(1.0L, -1.0L, -1.0L, 1.0L, 1.0L,
                                           -1.0L, -1.0L, 1.0L);
...
void applyPauliYDoubleExternal(std::complex<double> *arr,
                                      const size_t num_qubits,
                                      const size_t rev_wire) {
    const size_t rev_wire_shift = (static_cast<size_t>(1U) << rev_wire);
    const size_t wire_parity = fillTrailingOnes(rev_wire);
    ...
    for (size_t k = 0; k < exp2(num_qubits - 1);
        ...
        _mm512_store_pd(arr + i0, Util::productImagD(v1, _mm512_set1_pd(-1.0L)));
