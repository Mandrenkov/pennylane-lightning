...
/// @cond DEV
template <size_t rev_wire> inline void applyPauliXFloatInternalOp(__m256 &v) {
    if constexpr (rev_wire == 0) {
        v = _mm256_permute_ps(v, 0B01001110);
    } else if (rev_wire == 1) {
        const auto shuffle_idx = _mm256_set_epi32(11, 10, 9, 8, 15, 14, 13, 12,
                                                  3, 2, 1, 0, 7, 6, 5, 4);
        v = _mm256_permutexvar_ps(shuffle_idx, v);
    } else if (rev_wire == 2) {
        const auto shuffle_idx = _mm256_set_epi32(7, 6, 5, 4, 3, 2, 1, 0, 15,
                                                  14, 13, 12, 11, 10, 9, 8);
        v = _mm256_permutexvar_ps(shuffle_idx, v);
    }
}
...
inline void applyPauliXFloatExternal(std::complex<float> *arr,
    ...
    for (size_t k = 0; k < exp2(num_qubits - 1);
        ...
        _mm256_store_ps(arr + i0, v1);
        _mm256_store_ps(arr + i1, v0);
    }
}

template <size_t rev_wire> inline void applyPauliXDoubleInternalOp(__m256d &v) {
    if constexpr (rev_wire == 0) {
        //const auto shuffle_idx = _mm256_set_epi64(5, 4, 7, 6, 1, 0, 3, 2);
        // 1, 0, 3, 2
        v = _mm256_permutex_pd(v, 0B01'00'11'10);
    } else if (rev_wire == 1) {
        const auto shuffle_idx = _mm256_set_epi64(3, 2, 1, 0, 7, 6, 5, 4);
...
inline void applyPauliXDoubleExternal(std::complex<double> *arr,
    ...
    for (size_t k = 0; k < exp2(num_qubits - 1);
        ...
        _mm256_store_pd(arr + i0, v1);
        _mm256_store_pd(arr + i1, v0);
    }
}
/// @endcond
} // namespace Pennylane::Gates::AVX2
