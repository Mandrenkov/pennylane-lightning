...
template <typename PrecisionT, size_t packed_size> struct ApplyIsingXX {
    ...
    static constexpr auto permutationInternalInternal() {
        std::array<uint8_t, packed_size> perm = {0,};
    ...
    static void applyInternalInternal(std::complex<PrecisionT> *arr,
        ...
        constexpr static auto perm = permutationInternalInternal<rev_wire0, rev_wire1>();
    ...
    applyExternalExternal(std::complex<PrecisionT> *arr,
        ...
        const auto cos_factor = set1<PrecisionT, packed_size>(std::cos(angle / 2));
        const auto isin_factor = imagFactor<PrecisionT, packed_size>(isin);
