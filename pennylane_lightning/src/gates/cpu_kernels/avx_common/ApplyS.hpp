...
/// @cond DEV
namespace Internal {
template <int perm_size, int rev_wire> struct PauliSInternalPerm;

template <> struct PauliSInternalPerm<4, 0> {
    // rev_wire == 0
    using Type = Permute4<0, 1, 3, 2>;
};
template <> struct PauliSInternalPerm<8, 0> {
    // rev_wire == 0
    using Type = Permute8<0, 1, 3, 2, 4, 5, 7, 6>;
};
template <> struct PauliSInternalPerm<8, 1> {
    // rev_wire == 0
    using Type = Permute8<0, 1, 2, 3, 5, 4, 7, 6>;
};
template <> struct PauliSInternalPerm<16, 0> {
    // rev_wire == 0
    using Type =
        Permute16<0, 1, 3, 2, 4, 5, 7, 6, 8, 9, 11, 10, 12, 13, 15, 14>;
};
template <> struct PauliSInternalPerm<16, 1> {
    // rev_wire == 0
    using Type =
        Permute16<0, 1, 2, 3, 5, 4, 7, 6, 8, 9, 10, 11, 13, 12, 15, 14>;
};
template <> struct PauliSInternalPerm<16, 2> {
    // rev_wire == 0
    using Type =
        Permute16<0, 1, 2, 3, 4, 5, 6, 7, 9, 8, 11, 10, 13, 12, 15, 14>;
};

template <typename RealProd, int rev_wire> struct ApplySInternalFactor;

template <typename RealProd> struct ApplySInternalFactor<RealProd, 0> {
    static auto create() { return RealProd::setr4(1.0F, 1.0F, -1.0F, 1.0F); }
};
template <typename RealProd> struct ApplySInternalFactor<RealProd, 1> {
    static auto create() {
        return RealProd::setr8(1.0F, 1.0F, 1.0F, 1.0F, -1.0F, 1.0F, -1.0F,
                               1.0F);
    }
};
template <typename RealProd> struct ApplySInternalFactor<RealProd, 2> {
    static auto create() {
        return RealProd::setr16(1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F,
                                -1.0F, 1.0F, -1.0F, 1.0F, -1.0F, 1.0F, -1.0F,
                                1.0F);
    }
};

template <typename RealProd, int rev_wire> auto createInternalFactor() {
...
struct ApplyS {
    ...
    static void applyInternal(std::complex<PrecisionT> *arr,
        ...
        using Perm = typename Internal::PauliSInternalPerm<
            sizeof(typename PrecisionAVXConcept::IntrinsicType) /
                sizeof(PrecisionT),
            rev_wire>::Type;
