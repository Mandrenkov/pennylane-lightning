...
#include "avx512/ApplySingleQubitOp.hpp"
...
class GateImplementationsAVX512 {
  ...
  public:
    static void applySingleQubitOp(std::complex<float> *arr,
        ...
        switch (rev_wire) {
        case 0:
            AVX512::applySingleQubitOpInternal<0>(arr, num_qubits, matrix,
                                                  inverse);
            return;
        case 1:
            AVX512::applySingleQubitOpInternal<1>(arr, num_qubits, matrix,
                                                  inverse);
            return;
        case 2:
            AVX512::applySingleQubitOpInternal<2>(arr, num_qubits, matrix,
                                                  inverse);
            return;
        default:
            AVX512::applySingleQubitOpExternal(arr, num_qubits, rev_wire,
                                               matrix, inverse);
            return;
        }
    }
    ...
    static void applyIsingZZ(std::complex<PrecisionT> *arr,
        ...
        if constexpr (std::is_same_v<PrecisionT, float>) {
            ...
            const size_t rev_wire1 = num_qubits - wires[0] - 1; // Control qubit
            if (rev_wire0 < 3 && rev_wire1 < 3) {
                AVX512::applyIsingZZFloatInternalInternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            } else if (std::min(rev_wire0, rev_wire1) < 3) {
                AVX512::applyIsingZZFloatInternalExternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            } else {
                AVX512::applyIsingZZFloatExternalExternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            }
        } else if (std::is_same_v<PrecisionT, double>) {
            if (num_qubits < 2) {
            ...
            if (rev_wire0 < 2 && rev_wire1 < 2) {
                AVX512::applyIsingZZDoubleInternalInternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            } else if (std::min(rev_wire0, rev_wire1) < 2) {
                AVX512::applyIsingZZDoubleInternalExternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            } else {
                AVX512::applyIsingZZDoubleExternalExternal(
                    arr, num_qubits, rev_wire0, rev_wire1, inverse, angle);
            }
        } else {
            static_assert(std::is_same_v<PrecisionT, float> ||
