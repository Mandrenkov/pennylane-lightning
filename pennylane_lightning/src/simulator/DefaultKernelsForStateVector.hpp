...
class DefaultKernelsForStateVector {
  private:
    ...
    std::unordered_map<Gates::GateOperation, 
        std::vector<std::tuple<uint32_t, std::function<bool(size_t)>, Gates::KernelType>>> kernel_map_;
    ...
    void registerThreadsDefaultGates() {
        ...
        instance.assignKernelForGate(GateOperation::PauliX, Threading::All,
                CPUMemoryModel::All, all_qubit_numbers, Gates::KernelType::LM);
        ...
        instance.assignKernelForGate(GateOperation::Rot, Threading::All,
                CPUMemoryModel::All, all_qubit_numbers, Gates::KernelType::LM);
        instance.assignKernelForGate(GateOperation::ControlledPhaseShift, Threading::All,
                CPUMemoryModel::All, all_qubit_numbers, Gates::KernelType::LM);
        ...
        instance.assignKernelForGate(GateOperation::IsingXX, Threading::All,
                CPUMemoryModel::All, less_than(12), Gates::KernelType::LM);
        instance.assignKernelForGate(GateOperation::IsingXX, Threading::All,
                CPUMemoryModel::All, in_between_closed(12, 20), Gates::KernelType::PI);
        instance.assignKernelForGate(GateOperation::IsingXX, Threading::All,
                CPUMemoryModel::All, larger_than(20), Gates::KernelType::LM);
  ...
  public:
    static struct AllThreading {} all_threading;
    static struct AllMemoryModel {} all_memory_model;
    ...
    auto getKernelMap(Threading threading, CPUMemoryModel memory_model, 
            size_t num_qubits) -> std::unordered_map<Gates::GateOperation, Gates::KernelType> {
        uint32_t dispatch_key = toDispatchKey(threading, memory_model);

        std::unordered_map<Gates::GateOperation, Gates::KernelType> kernel_for_gates;

        for (auto gate = Gates::GateOperation::BEGIN;
                gate != Gates::GateOperation::END;
                gate = static_cast<Gates::GateOperation>(static_cast<uint32_t>(gate) + 1)) {
            ...
            if (max_dispatch_key == 0) {
                throw std::range_error("Cannot find registered kernel for a dispatch key and number of qubits.");
            }
